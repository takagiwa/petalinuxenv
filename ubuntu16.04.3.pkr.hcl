...
source "virtualbox-iso" "ubuntu" {
    ...
    boot_command = [
        ...
        "<bs><bs><bs><bs><bs><bs><bs><bs><bs><bs><wait>",
        "<bs><bs><bs>",
        "file=<wait>/cdrom/<wait>preseed/<wait>ubuntu-server.seed <wait><wait>auto-install/<wait>enable=<wait>true <wait><wait>debconf/<wait>priority=<wait>critical <wait><wait>preseed/url=http://<wait><wait>{{ .HTTPIP }}:{{ .HTTPPort }}<wait><wait>/ubuntu16.04.1.cfg <wait><wait>vga=788 <wait><wait>initrd=<wait>/install/initrd.gz ---<wait><enter><wait>"
    ]
...
build {
    ...
    post-processors {  
      post-processor "artifice" {
        files = [
          "build/packer-virtualbox/ubuntu16043-disk001.vmdk",
          "build/packer-virtualbox/ubuntu16043.ovf"
        ]
      }
